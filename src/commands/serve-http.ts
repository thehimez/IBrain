...
function getAllowedMobileReturnUrl(value: unknown): string | null {
  if (typeof value !== 'string' || value.length > 500) return null;
  try {
    const url = new URL(value);
    if (url.protocol === 'xandacross:' && url.hostname === 'auth') return MOBILE_RETURN_URL;
    if (url.protocol === 'exp:' && url.pathname.endsWith('/--/auth')) return value;
  } catch {
...
>>>>>>> origin/main
  ...
  // ---------------------------------------------------------------------------
  // Frontend UI — serve Vite build if present, otherwise redirect to /admin
  // ---------------------------------------------------------------------------
  const frontendDistPath = new URL('../../frontend/dist', import.meta.url).pathname;
  if (existsSync(frontendDistPath)) {
    app.use(express.static(frontendDistPath));
    app.get('/', (_req, res) => res.sendFile(`${frontendDistPath}/index.html`));
  } else {
...
<<<<<<< HEAD
      ...
      if (mobileReturnUrl) {
        ...
        mobileAuthCodes.set(code, { token, expiresAt: Date.now() + MOBILE_AUTH_CODE_TTL_MS });
...
<<<<<<< HEAD
  ...
  app.post('/api/auth/mobile-session', express.json(), (req: Request, res: Response) => {
    ...
    if (!entry || entry.expiresAt <= Date.now()) {
      res.status(401).json({ error: 'invalid_or_expired_mobile_code' });
      return;
    }
...
>>>>>>> origin/main
      ...
      res.status(503).json({ connected: false, name: 'Industrial Knowledge Brain', pageCount: 0, engine: 'unknown', version: 'unknown' });
    }
  });
  ...
  app.post('/api/chat', express.json(), async (req: Request, res: Response) => {
    try {
      ...
      let parsed: { answer?: string; citations?: unknown[]; gaps?: string[] } = {};
...
<<<<<<< HEAD
      ...
      // remains the source of truth and Postgres keeps its asynchronous path.
      if (config.engine === 'pglite') {
        ...
        let finalJob = await ingestQueue.getJob(job.id);
        while (
          finalJob &&
          !['completed', 'failed', 'dead', 'cancelled'].includes(finalJob.status) &&
          Date.now() < deadline
        ) {
          await new Promise(resolve => setTimeout(resolve, 100));
          finalJob = await ingestQueue.getJob(job.id);
        }
        worker.stop();
        await workerPromise;

        if (finalJob?.status !== 'completed') {
          throw new Error(finalJob?.error_text ?? 'Timed out while processing uploaded document');
        }
      }
