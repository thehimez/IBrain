...
export default function SourceChips({ citations, gaps }: Props) {
  ...
  return (
    <View style={{ gap: 6 }}>
      {unique.length > 0 && (
        <View>
          <Text style={{ fontSize: 10, color: Colors.text.muted, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>
            Sources
          </Text>
            ...
            <View style={{ flexDirection: 'row', gap: 6 }}>
              {unique.map((c, i) => (
                ...
                >
                  ...
                  <Text
                    style={{ fontSize: 11, color: Colors.accent.light, maxWidth: 120 }}
                    numberOfLines={1}
                  >
                    {c.page_slug}
                  </Text>
